...
namespace Infraestructure.Filters
{
    public class GlobalExceptionFilter(ILogger<GlobalExceptionFilter> logger) : IExceptionFilter
    {
        ...
        public void OnException(ExceptionContext context)
        {
            ...
            if (context.Exception is BusinessException exception) {
                var response = new Response {
                    ...
                    Message = string.IsNullOrEmpty(exception.MessageStatus) ? ReasonPhrases.GetReasonPhrase((int)exception.Status) : exception.MessageStatus,
        ...
        {
            _logger.LogError(
                "Ha ocurrido la siguiente excepción: {message}, con el detalle: {detail}, en la hora: {time} con",
